    ...
    if mode == "premium":
        ...
        d1.execute(
            ...
            [str(uuid.uuid4()), agent, task_type, "premium",
             winner["model_id"], runner_up["model_id"] if runner_up else None,
             round(float(winner.get("avg_quality") or 0), 4),
             round(float(winner.get("win_rate") or 0), 3),
             cost_val, winner["n_trials"], run_id, date_str, status, now_ts])
...
def run_smoke_tests(provider_filter=None, agent_filter=None, task_type_filter=None, mode="standard"):
    ...
    summary = {"run_id": run_id, "mode": mode,
               ...
               "scorer": SCORER_MODEL, "slot_config": slot_cfg,
    ...
    with D1Client(cfg) as d1, R2Client(cfg) as r2:
        ...
        models = d1.query_rows(
            ...
            params)
        log.info(f"Loaded {len(models)} models | Mode: {mode.upper()} | Scorer: {SCORER_MODEL} | Pool: {slot_cfg}")
        if mode == "premium":
            active_premium = [m for m in models if m["model_id"] in PREMIUM_MODELS]
            log.info(f"Premium candidates: {[m['model_id'] for m in active_premium]}")
            ...
            for task_type, cases in task_map.items():
                ...
                if mode == "standard":
                    ...
                    if ps and ps[0].get("status") == "converged":
                        log.info(f"SKIP [{agent}/{task_type}] — premium converged"); continue
                ...
                for model in slot_models:
                    ...
                    if model_rewards and not cfg.dry_run:
                        ...
                        d1.execute(
                            ...
                            [str(uuid.uuid4()),agent,task_type,model_id,mode,
