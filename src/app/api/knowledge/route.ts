...
export async function POST(req: NextRequest) {
  ...
  if (type === 'study_node') {
    ...
    ).bind(id, data.title, data.category, data.content,
           data.tags ?? null, data.source ?? null, now, now).run();
  } else {
    await DB.prepare(
    ...
    ).bind(id, data.sprint_number, data.title, data.description ?? null,
           data.status ?? 'todo', data.priority ?? 2,
           data.agent ?? null, data.category ?? null, now, now).run();
  }

  const embedContent = buildEmbedContent(type, { ...data, id });
  await KNOWLEDGE_QUEUE.send({
    id,
    table,
    content: embedContent,
    metadata: { type, title: data.title },
  });
