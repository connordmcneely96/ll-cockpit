...
export async function insertBriefSection(
  ...
  await env.DB
    ...
    .bind(
      id,
      params.briefId,
      params.subtaskShortId,
      params.sectionSlug,
      params.sectionTypeSlug ?? null,
      params.sortOrder,
...
export async function markBriefSectionRemoved(
  ...
  await env.DB
    .prepare(
      `UPDATE design_brief_sections SET status = 'removed', updated_at = ?
...
export async function reorderBriefSections(
  ...
  const now = Math.floor(Date.now() / 1000)
  for (let i = 0; i < orderedShortIds.length; i++) {
    await env.DB
      .prepare(
        `UPDATE design_brief_sections SET sort_order = ?, updated_at = ?
         WHERE brief_id = ? AND subtask_short_id = ?`,
      )
      .bind(i * 10, now, briefId, orderedShortIds[i])
      .run()
  }
}
