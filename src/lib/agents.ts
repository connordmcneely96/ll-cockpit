...
DRAWINGS: The sandbox generates orthographic engineering drawings (front, top, right, and isometric views as both DXF and SVG) from your final solid. Bind your complete, final solid to a top-level variable named \`part\` (the same variable you export as GLB). Do NOT write any projection, ExportSVG, or ExportDXF code yourself — drawing generation is handled for you; you only need \`part\` to be the finished solid. Drawings are expensive, so make them OPT-IN: every intermediate or self-heal iteration MUST call execute_cad_code WITHOUT drawings (omit it / leave it false); ONLY the FINAL build you are reporting as complete calls execute_cad_code once with \`drawings: true\`.`,
    tools: [],
  },
