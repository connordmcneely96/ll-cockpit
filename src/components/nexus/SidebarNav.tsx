...
export default function SidebarNav({ navGroups }: Props) {
  ...
  return (
    ...
    >
      <div className="px-4 py-4 shrink-0" style={{ borderBottom: '1px solid var(--t-glass-bdr)' }}>
        <div className="text-sm font-bold tracking-wide" style={{ fontFamily: 'var(--font-condensed), sans-serif', fontWeight: 700, color: 'var(--t-tx1)', letterSpacing: '0.04em' }}>
          NEXUS PRIME
        </div>
        <div className="text-xs mt-0.5" style={{ color: 'var(--t-tx3)', letterSpacing: '0.12em' }}>
          AI OPERATING SYSTEM
        </div>
      </div>
      <nav className="flex-1 px-2 py-3 flex flex-col gap-4">
        {navGroups.map(group => (
          <div key={group.id}>
            <div className="px-2 mb-1 text-xs font-semibold uppercase" style={{ color: 'var(--t-tx3)', letterSpacing: '0.08em' }}>
              {group.label}
            </div>
              ...
              {group.items.map(item => {
                ...
                return (
                  <li key={item.id}>
                    ...
                    >
                      {isActive && (
                        <span className="inline-block rounded-full shrink-0" style={{ width: 5, height: 5, background: 'var(--t-p)' }} />
                      )}
                      {item.label}
                    </Link>
                  </li>
