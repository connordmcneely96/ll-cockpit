...
export function AgentPanel() {
  ...
  const rawMessages = useAgentStore((s) =>
    selectedAgent ? s.sessions[selectedAgent] : undefined
  )
  const messages = rawMessages ?? EMPTY_MESSAGES
  const isStreaming = useAgentStore((s) => s.isStreaming)

  const { sendMessage } = useAgentStream(selectedAgent ?? '')
      ...
      {!agent ? (
        ...
        <>
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-6 text-center gap-3">
            <div className="w-12 h-12 rounded-full bg-base-3 border border-white/[0.06] flex items-center justify-center text-xl">
              🤖
            </div>
            <p className="text-text3 font-mono text-xs">Select an agent</p>
            <p className="text-text3/50 font-mono text-[10px] leading-relaxed">
              Click any agent in the explorer to begin a conversation
            </p>
          </div>
        </>
      ) : (
        ...
        <>
          ...
          <div className="h-10 shrink-0 bg-base-2 border-b border-white/[0.06] flex items-center justify-between px-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="text-text3 hover:text-text2 font-mono text-sm transition-colors leading-none"
              >
                ×
              </button>
            </div>
          ...
          <div className="h-8 shrink-0 bg-base-2 border-b border-white/[0.06] flex items-end px-3 gap-4">
            <button className="text-text1 border-b-2 border-cyan -mb-px font-mono text-[10px] pb-1.5 transition-colors">
              Chat
            </button>
            <button className="text-text3 hover:text-text2 font-mono text-[10px] pb-1.5 transition-colors">
              History
            </button>
          </div>
