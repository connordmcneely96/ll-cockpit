...
import { LayoutDashboard, Code2, TerminalSquare, HardDrive, BarChart2, Bot, GitBranch, Settings } from 'lucide-react'
...
export function ActivityRail() {
  ...
  return (
    <aside className="flex flex-col items-center shrink-0 h-full py-2 gap-0.5" style={{
      width: 48,
      ...
      <div className="w-8 h-8 rounded-xl flex items-center justify-center mb-3 shrink-0" style={{
        background: 'var(--t-p-glass)', border: '1px solid var(--t-glass-bdr)',
      ...
      {NAV_ITEMS.map(({ icon: Icon, label, href }) => {
        ...
        return (
          <Link key={href} href={href} title={label}
            ...
            <span className="absolute left-10 z-50 px-2 py-1 rounded-lg text-[10px] font-mono whitespace-nowrap pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity"
              style={{ background: 'var(--d-elevated)', backdropFilter: 'blur(20px)', border: '1px solid var(--t-glass-bdr)', color: 'var(--t-tx1)' }}>
              {label}
            </span>
            {isActive && <span className="absolute left-0 top-1/2 -translate-y-1/2 rounded-r" style={{ width:2,height:16,background:'var(--t-p)',boxShadow:'0 0 6px var(--t-p-glow)' }} />}
          </Link>
        )
      })}
      <div className="flex-1" />
      <Link href="/settings" title="Settings" className="w-8 h-8 rounded-xl flex items-center justify-center transition-all" style={{ color: 'var(--t-tx3)' }}>
