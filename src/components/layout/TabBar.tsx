...
import { LayoutDashboard, Code2, Globe, TerminalSquare, Cpu, Bot, HardDrive, BarChart2, X } from 'lucide-react'
...
export function TabBar() {
  ...
  return (
    <div className="flex items-end shrink-0 overflow-x-auto" style={{
      height: 35,
      ...
      {TABS.map(({ icon: Icon, label, href }) => {
        ...
        return (
          <Link key={href} href={href}
            ...
            style={{ background: isActive ? 'var(--t-p-glass)' : 'transparent', borderRight:'1px solid var(--t-glass-bdr)', color: isActive ? 'var(--t-tx1)' : 'var(--t-tx3)' }}>
            {isActive && <span className="absolute top-0 left-0 right-0" style={{ height:2, background:'linear-gradient(to right, var(--t-p), var(--t-p-bright))', boxShadow:'0 0 8px var(--t-p-glow)' }} />}
            ...
            <span className="ml-0.5 opacity-0 group-hover:opacity-60 hover:!opacity-100 transition-opacity" style={{ color:'var(--t-tx3)' }} onClick={e=>e.preventDefault()}><X size={10} /></span>
          </Link>
        )
      })}
      <div className="flex-1" style={{ height:'100%', borderBottom:'1px solid var(--t-glass-bdr)' }} />
    </div>
  )
}
