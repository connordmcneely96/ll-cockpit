...
export function DesignProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    // ── 1. Base theme CSS vars ──
    Object.entries(theme.vars).forEach(([k, v]) => root.style.setProperty(k, v))
    root.setAttribute('data-theme', activeThemeId)
    root.setAttribute('data-mode', isDark ? 'dark' : 'light')

    // ── 2. Sync body + gradients from theme on theme switch ──
    if (d.syncWithTheme && activeThemeId !== prevThemeId.current) {
      ...
      return // re-renders with synced values
    }
    prevThemeId.current = activeThemeId
    ...
    // ── 4. Derive panel fill color from body + elevation ──
    // All surfaces are tinted versions of this single derived color.
    // Lowering alpha on any surface reveals the background gradient through it.
    const panelHex = derivePanelHex(d.bgBase, isDark, d.surfaceElevation)

    // ── 5. Per-surface CSS vars ──
    // Each UI layer has its own alpha — independently controllable.
    root.style.setProperty('--d-rail',     hexAlpha(panelHex, d.railAlpha))      // ActivityRail
    root.style.setProperty('--d-explorer', hexAlpha(panelHex, d.explorerAlpha))  // ExplorerPanel
    root.style.setProperty('--d-topbar',   hexAlpha(panelHex, d.topbarAlpha))    // TopBar
    root.style.setProperty('--d-tabbar',   hexAlpha(panelHex, d.tabbarAlpha))    // TabBar
    root.style.setProperty('--d-agent',    hexAlpha(panelHex, d.agentAlpha))     // AgentPanel
    root.style.setProperty('--d-panel',    hexAlpha(panelHex, d.panelAlpha))     // generic
    root.style.setProperty('--d-card',     hexAlpha(panelHex, d.cardAlpha))      // cards
    root.style.setProperty('--d-elevated', hexAlpha(panelHex, d.elevatedAlpha))  // dropdowns/modals
    // Keep --t-panel for legacy components
    ...
    root.style.setProperty('--t-shadow', buildShadowCSS(d.shadowPreset, d.shadowLayers, 1, sh, sa, d.innerHighlight, d.innerHighlightOpacity))
    root.style.setProperty('--t-shadow-hover', buildShadowCSS(d.shadowPreset, d.shadowLayers, 1.6, sh, sa, d.innerHighlight, d.innerHighlightOpacity))
